...
class BitGrid
{
	...
	BitGrid(float world_width, float world_height, size_t cols, size_t rows) :
	world_width(world_width), world_height(world_height), cols(cols), rows(rows),
	cell_width(world_width/cols), cell_height(world_height/rows) {
		rowbits= new Bit[rows];
		colbits= new Bit[cols]; 
	}
	...
	size_t findRowPosition ( float y )
	{
		return clamp((int)floor(y / cell_height), 0, (int)(rows - 1));
	}
	
	size_t findColumnPosition ( float x )
	{
		return clamp((int)floor(x / cell_width), 0, (int)(cols - 1));
	}
	

	
	//puts objects into its respective bitsets
	void put ( Circle &c )
	{
		//find row and column position of the circle
		const size_t row_loc = findRowPosition(c.center.y), 
		col_loc = findColumnPosition(c.center.x);
...
public:
	...
	vector<Circle*>& getCellForPosition(float x, float y) {
		size_t idx = getCellIndexForPosition(x, y);
		return grid[idx];
	}
	...
	vector<Circle*> getObjectsInRect(float centerX, float centerY, float w, float h) {
		...
		for ( int c = x_min; c <= x_max; ++c ) {
			for ( int r = y_min; r <= y_max; ++r ) {
				const vector<Circle*> &cell = getCell(c, r);
...
{
	float screenWidth = this->getWindowWidth(), screenHeight = this->getWindowHeight();
...
void bouncierApp::update()
{
	...
	for ( size_t i = 0; i < N_BALLS; ++i ) {
		Circle &c = circles[i];
		const Vector2 prevCenter = c.center;
		c.center += c.vel;
		...
		vector<Circle*> candidates = ug->getObjectsInRect(prevCenter.x, prevCenter.y, ug->cellWidth() * 2, ug->cellHeight() * 2);
		...
		for ( size_t j = 0; j < candidates.size(); ++j ) {
			...
			if ( dsq <= (c.radius + o.radius) * (c.radius + o.radius) ) {
				...
				float invTotalMass = 1.0f/(o.mass() + c.mass());
				...
				float w = (c.radius + o.radius - mag) * 0.5;
				
				const Vector2 prevOCenter = o.center;
				o.center += d * w;
				c.center -= d * w;
				
				//update the bin of o
				vector<Circle*> &OoldCell = ug->getCellForPosition(prevOCenter.x, prevOCenter.y);
				OoldCell.erase(find(OoldCell.begin(), OoldCell.end(), &o));
				ug->put(o);
			}
		}
		//update the bin of c
		vector<Circle*> &oldCell = ug->getCellForPosition(prevCenter.x, prevCenter.y);
